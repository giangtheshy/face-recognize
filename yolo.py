...
def detect_faces(detection_queue, model_name, gpu_id):
    ...
    while True:
        ...
        if item is None:
            ...
            break
        task_id, current_time_sec, frame = item
                ...
                if hasattr(result, 'boxes') and len(result.boxes) > 0:
                    for detection in result.boxes:
                        if detection.cls == 0:  # Giả định lớp 0 là khuôn mặt
                            ...
                            face_filename = os.path.join(faces_dir, f"{int(current_time_sec)}.png")
                            cv2.imwrite(face_filename, face_image)
                            logging.info(f"Saved face image: {face_filename}")
    ...
    try:
        ...
        while True:
            ...
            if frame_count % frame_interval == 0:
                ...
                detection_queue.put((task_id, current_time_sec, frame))
                current_time_sec += 1
...
def main():
    ...
    num_detection_processes = 10  # Số lượng process cho phát hiện khuôn mặt
    detection_queue = JoinableQueue()

    # Khởi tạo các process phát hiện khuôn mặt
    detection_processes = []
    for i in range(num_detection_processes):
        p = Process(target=detect_faces, args=(detection_queue, model_name, i%3), name=f"Detection-Process-{i+1}")
        p.start()
