    ...
    if results:
        for idx, embedding_dict in enumerate(results):
            ...
            if region:
                ...
                top, left, bottom, right = 0, 0, frame.shape[0], frame.shape[1]
                print(f"[{current_process().name}] Không có thông tin vị trí khuôn mặt, sử dụng toàn bộ khung hình.")
        ...
        print(f"[{current_process().name}] Không phát hiện khuôn mặt trong hình ảnh {image_path}.")
